...
async function start() {
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        "Failed to send RECORDER_STARTED message, continuing anyway:",
        e
      );
    }
    ...
    const details =
      e && typeof e === "object"
        ? `${e.name || "DOMException"}: ${e.message || e}`
        : String(e);
    status.textContent =
      "Failed to start: " +
      details +
      ". Ensure this tab is focused and click Start again.";
...
window.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .addEventListener("click", start, { once: false });
