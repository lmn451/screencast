...
async function render() {
  try {
    ...
    recordings.forEach(rec => {
      ...
      meta.className = 'meta';
      meta.textContent = `${rec.mimeType || 'video/webm'} • ${formatDuration(rec.duration)} • ${formatSize(rec.size)}`;
      ...
      playBtn.onclick = () => {
        chrome.tabs.create({ url: `preview.html?id=${encodeURIComponent(rec.id)}` });
      };
