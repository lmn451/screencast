...
Add-ons submission for ScreenSilo 0.2.3.
...
2. `scripts/package.sh firefox` copies the explicit Firefox production
   ...
   `dist/screensilo-firefox-mv3-0.2.3.zip`.
...
```sh
pnpm run typecheck
unzip -l dist/screensilo-firefox-mv3-0.2.3.zip
```

The source archive contains the files needed to reproduce the submitted build.
Run the full test and lint suites from the GitHub repository, which also includes
the test fixtures and development configuration.
