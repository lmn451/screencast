...
describe('recording.js additional tests', () => {
  ...
  it('finishRecording then getRecording returns metadata and blob', async () => {
    ...
    await finishRecording('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'video/webm', 1500, 2048);

    const rec = await getRecording('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa');
    expect(rec).not.toBeNull();
    expect(rec.id).toBe('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa');
  ...
  it('getRecording returns null when metadata missing', async () => {
    ...
    const res = await getRecording('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb');
    expect(res).toBeNull();
  });

  it('updateRecordingName rejects when recording not found', async () => {
    const { updateRecordingName } = await import('../../recording.js');
    await expect(updateRecordingName('cccccccc-cccc-cccc-cccc-cccccccccccc', 'new name')).rejects.toThrow('Recording not found');
  });
});
