/**
 * E2E Test: Yahoo.com - Type hello world
...
test('yahoo.com - type hello world', async ({ context }) => {
  const timestamp = Date.now();
  const outputPath = path.join(OUTPUT_DIR, `yahoo-${timestamp}.webm`);
  
  console.log(`
========================================
 Yahoo.com Test - Type "hello world"
========================================
Output: ${outputPath}
========================================
`);
  ...
  console.log('[1/7] Navigating to yahoo.com...');
  await page.goto('https://www.yahoo.com');
  await page.waitForLoadState('networkidle');
  console.log('[2/7] Page loaded');

  // Inject recording with getDisplayMedia
  ...
  const result = await page.evaluate(async () => {
    ...
    window.__recorder.ondataavailable = (e) => {
      ...
      console.log('[Page] Chunk:', e.data.size);
    };
  ...
  try {
    const searchBox = page.locator('input[name="p"]').first();
    await searchBox.click({ timeout: 5000 });
    await searchBox.fill('hello world');
    console.log('[5/7] Typed in Yahoo search box');
  } catch (e) {
    console.log('[5/7] Click failed, trying keyboard');
  ...
  // Take screenshot
  const screenshotPath = path.join(OUTPUT_DIR, `yahoo-${timestamp}.png`);
