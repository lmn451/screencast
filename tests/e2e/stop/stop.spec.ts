...
async function generateWebmBlobInPage(page) {
  return await page.evaluate(async () => {
    ...
    const id = setInterval(() => {
      ...
      ctx.fillStyle = '#0f0';
      ctx.fillRect((t * 10) % 320, 60, 40, 40);
    }, 1000 / fps);
...
test.describe('Stop feature (offscreen strategy) without code changes', () => {
  test('explicit STOP produces preview and data via message-only flow', async ({
    context,
    extensionId,
  }) => {
    ...
    const recordingId = await controlPage.evaluate(
      () =>
  ...
  test('auto-stop behavior simulated by delivering OFFSCREEN_DATA without STOP', async ({
    context,
    extensionId,
  }) => {
    const controlPage = await context.newPage();
